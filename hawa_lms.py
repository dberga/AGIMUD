...
def show_tutorial():
    """Display the tutorial/installation guide"""
    tutorial = """
+================================================================================+
|                         LM STUDIO PYTHON SDK TUTORIAL                          |
+================================================================================+
...
--------------------------------------------------------------------------------
  ...
  2. Run LM Studio GUI and download a specific model, OR run in terminal:
     $ lms get qwen/qwen3-4b-2507

  3. Install LM Studio Python SDK:
     $ pip install lmstudio

  4. Check the official documentation for more:
     https://lmstudio.ai/docs/python

  5. Run this script with various options (see --help for examples)
...
--------------------------------------------------------------------------------
* WITH MCP INTEGRATIONS (tools):
--------------------------------------------------------------------------------
  # Use web-search MCP tool
  python hawa_lms.py --mode sync_stream --mcp "mcp/web-search" -q "Search for AI news"
  
  # Use multiple MCP tools
  python hawa_lms.py --mode sync_stream --mcp "mcp/playwright" --mcp "mcp/web-search" -q "Search for AI news"
  
  # With LM Studio presets
  python hawa_lms.py --mode sync_stream --mcp "mcp/web-search" -q "What's the latest on quantum computing?" --preset "web_search.preset.json"
...
--------------------------------------------------------------------------------
* IMAGES USAGE (multimodal models):
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
  -m, --model          Model key to use
  -q, --question       Question to ask the model
  --mode               Mode: async, sync, last, complete, history, 
                       prepare_image, file, async_stream, sync_stream, 
                       last_stream, complete_stream, file_stream
  -l, --list           List all downloaded models
  -p, --prompt         Prompt for completion mode
  -s, --system-prompt  System prompt to set model behavior
  --preset             Path to .preset.json file
  --mcp                MCP server integration (can be used multiple times)
  --history            Chat history as alternating user/assistant messages
  -i, --image          Path to image file for prepare_image mode
  -f, --file           Path to text file for file mode
  --no-reasoning       Disable displaying reasoning/thinking process
  --temperature        Temperature for prediction (0.0 to 2.0)
  --max-tokens         Maximum number of tokens to generate
  --top-p              Top-p sampling parameter (0.0 to 1.0)
  --top-k              Top-k sampling parameter
  --repeat-penalty     Repeat penalty (1.0 = no penalty)
  --seed               Random seed for reproducible results
  --stop               Stop sequences to halt generation
  --context-length     Context length for the model
  --gpu-offload        GPU offload ratio (0.0 to 1.0)
  --cache-type         Cache type: auto, f16, q4_0, q4_1, q5_0, q5_1, q8_0
...
class LMChat:
    ...
    def set_mcp_servers(self, servers: List[str]) -> None:
        """Set MCP servers to use for predictions"""
        self.mcp_servers = servers
        if servers:
            print(f"MCP integrations enabled: {', '.join(servers)}")
    
    def _apply_mcp_to_config(self, config: Optional[dict]) -> Optional[dict]:
        """Apply MCP servers to prediction config if any are set"""
        ...
        # Add integrations field with MCP servers
        config["integrations"] = self.mcp_servers.copy()
        return config
    ...
    def complete(self, prompt: str, predict_config: Optional[dict] = None) -> str:
        """Simple completion (non-streaming) with MCP support"""
        model = self.get_model()
        print(f"Prompt: {prompt}")
        print("Thinking...")
        
        # Apply MCP to config
        predict_config = self._apply_mcp_to_config(predict_config)
        if predict_config:
    ...
    def stream_complete(self, prompt: str, predict_config: Optional[dict] = None) -> None:
        """Stream a completion in real-time with MCP support"""
        model = self.get_model()
        print(f"Prompt: {prompt}")
    ...
    def sync_chat(self, question: str, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> str:
        """Synchronous chat with the model (non-streaming) with MCP support"""
    ...
    def sync_stream_chat(self, question: str, show_reasoning: bool = True, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> None:
        """Synchronous chat with streaming response - handles DeepSeek reasoning properly with MCP support"""
        with lms.Client() as client:
            model_key = self.model_key or self.get_last_model()
            print(f"Loading model: {model_key}")
            if self.load_config:
    ...
    async def async_chat(self, question: str, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> str:
        """Asynchronous chat with the model (non-streaming) with MCP support"""
    ...
    async def async_stream_chat(self, question: str, show_reasoning: bool = True, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> None:
        """Asynchronous chat with streaming response - handles DeepSeek reasoning properly with MCP support"""
        async with lms.AsyncClient() as client:
            model_key = self.model_key or self.get_last_model()
            print(f"Loading model: {model_key}")
            if self.load_config:
    ...
    def convenience_chat(self, question: str, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> str:
        """Convenience method using lms.llm() (non-streaming) with MCP support"""
        model = self.get_model()
        print(f"Question: {question}")
    ...
    def convenience_stream_chat(self, question: str, show_reasoning: bool = True, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> None:
        """Convenience method with streaming - handles DeepSeek reasoning properly with MCP support"""
        model = self.get_model()
        print(f"Question: {question}")
    ...
    def chat_with_history(self, messages: List[Tuple[str, str]], system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> str:
        """Chat with conversation history with MCP support"""
        model = self.get_model()
        ...
        for user_msg, assistant_msg in messages:
            ...
            response = model.respond(chat, config=predict_config)
    ...
    def chat_with_file(self, file_path: str, question: str, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> str:
        """Chat about a text file's contents with MCP support"""
    ...
    def stream_chat_with_file(self, file_path: str, question: str, show_reasoning: bool = True, system_prompt: Optional[str] = None, predict_config: Optional[dict] = None) -> None:
        """Chat about a text file's contents with streaming and MCP support"""
        content = self.read_text_file(file_path)
        prompt = f"Here is the content of a file:\n\n{content}\n\nQuestion: {question}"
        self.convenience_stream_chat(prompt, show_reasoning, system_prompt, predict_config)
    
def load_preset(preset_path: str) -> Optional[str]:
    """Load a preset file and extract the system prompt"""
    ...
    # Extract system prompt from the preset
    for field in preset_data.get("operation", {}).get("fields", []):
...
def build_prediction_config(args):
    ...
    # Note: MCP integrations will be added in _apply_mcp_to_config()
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Chat with LM Studio models with MCP support",
...
Examples:
  ...
  # Chat with MCP tools (must be configured in LM Studio mcp.json)
  python hawa_lms.py --mode sync_stream --mcp "mcp/playwright" --mcp "mcp/web-search" -q "Search for AI news"
  ...
  python hawa_lms.py --mode sync_stream -q "Explain AI" --system-prompt "You are a coding expert." --temperature 0.7 --max-tokens 500 --top-p 0.95 --repeat-penalty 1.1 --context-length 8192 --gpu-offload 0.5 --mcp "mcp/web-search"
  
  # Show tutorial/installation guide
  python hawa_lms.py --tutorial
  
  # Show this help with all examples
  python hawa_lms.py --help
        """
    )
    ...
    parser.add_argument(
        "--tutorial",
        action="store_true",
        help="Show the installation tutorial and setup guide"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a .preset.json file to load system prompt from"
    )
    ...
    parser.add_argument(
        ...
        action='append',
        default=[],
        help="MCP server integration to use (e.g., 'mcp/playwright', 'mcp/web-search'). Can be specified multiple times."
    )
...
def run():
    ...
    if args.mcp:
        chat.set_mcp_servers(args.mcp)
    if args.preset:
        preset_prompt = load_preset(args.preset)
        if preset_prompt:
            system_prompt = preset_prompt  # Override system prompt with preset
            print(f"Loaded preset from: {args.preset}")
    if args.list:
        chat.list_models()
        return
    ...
    try:
        ...
        if args.mode == "prepare_image":
            ...
            result = chat.chat_with_history(messages, system_prompt=system_prompt, predict_config=predict_config)
